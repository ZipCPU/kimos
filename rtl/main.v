...
// }}}
	...
	wire	[21:0]	wbwide_crossflash_addr;
	...
	wire	[21:0]	wbwide_crossbus_addr;
	...
	//
	wbxbar #(
		.NM(3), .NS(3), .AW(22), .DW(512),
		.SLAVE_ADDR({
			// Address width    = 22
			// Address LSBs     = 6
			{ 22'h280000 }, //      bkram: 0xa000000
			{ 22'h200000 }, //   crossbus: 0x8000000
			{ 22'h000000 }  // crossflash: 0x0000000
		}),
		.SLAVE_MASK({
			// Address width    = 22
			// Address LSBs     = 6
...
`ifdef	BKRAM_ACCESS
	...
	/*
	// Verilator lint_off UNUSED
	(* keep *) reg	[511:0]	mem_test;
	// Verilator lint_on  UNUSED
	always @(posedge i_clk)
		mem_test <= bkrami.mem[0];
	*/
	// }}}
`else	// BKRAM_ACCESS
	// {{{
...
`ifdef	SDIO_ACCESS
	...
	sdio #(
		...
		.o_dma_cyc(wbwide_sdio_cyc), .o_dma_stb(wbwide_sdio_stb), .o_dma_we(wbwide_sdio_we),
			.o_dma_addr(wbwide_sdio_addr[22-1:0]),
