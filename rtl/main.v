...
// }}}
	...
	//
	wire	ramscope_int;	// ramscope.INT.SDIOSCOPE.WIRE
...
// }}}
	...
	// Verilator lint_on UNUSED
	// Wishbone definitions for bus wb32(DIO), component migperf
	// Verilator lint_off UNUSED
	wire		wb32_migperf_cyc, wb32_migperf_stb, wb32_migperf_we;
	wire	[11:0]	wb32_migperf_addr;
	wire	[31:0]	wb32_migperf_data;
	wire	[3:0]	wb32_migperf_sel;
	wire		wb32_migperf_stall, wb32_migperf_ack, wb32_migperf_err;
	wire	[31:0]	wb32_migperf_idata;
	// Verilator lint_on UNUSED
	...
	// Verilator lint_on UNUSED
	// Wishbone definitions for bus wb32, component ramscope
	// Verilator lint_off UNUSED
	wire		wb32_ramscope_cyc, wb32_ramscope_stb, wb32_ramscope_we;
	wire	[11:0]	wb32_ramscope_addr;
	wire	[31:0]	wb32_ramscope_data;
	wire	[3:0]	wb32_ramscope_sel;
	wire		wb32_ramscope_stall, wb32_ramscope_ack, wb32_ramscope_err;
	wire	[31:0]	wb32_ramscope_idata;
	// Verilator lint_on UNUSED
	...
	always @(posedge i_clk)
	casez(wb32_dio_addr[5:4])
	2'b00: r_wb32_dio_bus_select <= 2'd0;
	2'b01: r_wb32_dio_bus_select <= 2'd1;
	2'b10: r_wb32_dio_bus_select <= 2'd2;
	2'b11: r_wb32_dio_bus_select <= 2'd3;
	...
	2'd2: r_wb32_dio_data <= wb32_migperf_idata;
	...
	assign	wb32_i2cs_stb = wb32_dio_stb && ((wb32_dio_addr[ 5: 4] &  2'h3) ==  2'h0);  // 0x000 - 0x00f
	...
	assign	wb32_i2cdma_stb = wb32_dio_stb && ((wb32_dio_addr[ 5: 4] &  2'h3) ==  2'h1);  // 0x040 - 0x04f
	...
	assign	wb32_migperf_cyc = wb32_dio_cyc;
	assign	wb32_migperf_stb = wb32_dio_stb && ((wb32_dio_addr[ 5: 4] &  2'h3) ==  2'h2);  // 0x080 - 0x0bf
	assign	wb32_migperf_we  = wb32_dio_we;
	assign	wb32_migperf_addr= wb32_dio_addr;
	assign	wb32_migperf_data= wb32_dio_data;
	assign	wb32_migperf_sel = wb32_dio_sel;
	assign	wb32_sio_cyc = wb32_dio_cyc;
	assign	wb32_sio_stb = wb32_dio_stb && ((wb32_dio_addr[ 5: 4] &  2'h3) ==  2'h3);  // 0x0c0 - 0x0ff
	...
	wbxbar #(
		...
		.SLAVE_ADDR({
			...
			{ 12'h300 }, //      cfg: 0x0c00
			{ 12'h280 }, //     uart: 0x0a00
			{ 12'h200 }, // ramscope: 0x0800
			{ 12'h180 }, // flashdbg: 0x0600
...
`ifdef	RAMSCOPE_SCOPC
	...
	wbscopc #(
		...
		.i_trigger(ram_debug[31]), .i_data(ram_debug[30:0]),
		.i_wb_clk(i_clk),
		.i_wb_cyc(wb32_ramscope_cyc), .i_wb_stb(wb32_ramscope_stb), .i_wb_we(wb32_ramscope_we),
			.i_wb_addr(wb32_ramscope_addr[1-1:0]),
			.i_wb_data(wb32_ramscope_data), // 32 bits wide
			.i_wb_sel(wb32_ramscope_sel),  // 32/8 bits wide
		.o_wb_stall(wb32_ramscope_stall),.o_wb_ack(wb32_ramscope_ack), .o_wb_data(wb32_ramscope_idata),
		.o_interrupt(ramscope_int)
		// }}}
	);
...
`else
			wbwide_ddr3_ack,
`endif
			// For targetting the MIG, we really want to see:
			// awvalid, awreday, wvalid, wready, bvalid, bready,
			//	arvalid, arready, rvalid, rready => 10sigs
			// These are necessary to check AXI transactions.
			//   However, we have ... only 7 open spots available
			10'h0,
			// mig_debug[9:0],
			// Peek at the CPU
			// {{{
			swic.thecpu.core.alu_pc_valid,
			swic.thecpu.core.alu_valid,
			//
			swic.thecpu.core.o_mem_ce,
			swic.thecpu.core.i_mem_busy,
			swic.thecpu.core.i_mem_rdbusy,
			swic.thecpu.core.i_mem_valid,
			// }}}
			// CPU's bus outputs
			// {{{
			wbwide_zip_cyc,
...
`else	// RAMSCOPE_SCOPC
	...
	//
	// In the case that there is no wb32_ramscope peripheral
	// responding on the wb32 bus
	assign	wb32_ramscope_ack   = 1'b0;
	assign	wb32_ramscope_err   = (wb32_ramscope_stb);
	assign	wb32_ramscope_stall = 0;
	assign	wb32_ramscope_idata = 0;

	// }}}
	// Null interrupt definitions
	// {{{
	assign	ramscope_int = 1'b0;	// ramscope.INT.SDIOSCOPE.WIRE
	// }}}
	// }}}
`endif	// RAMSCOPE_SCOPC
...
`ifdef	WBPERF_ACCESS
	...
	wbperf	#(
		...
		// {{{
		.i_clk(i_clk), .i_reset(i_reset),
		.i_wb_cyc(wb32_migperf_cyc), .i_wb_stb(wb32_migperf_stb), .i_wb_we(wb32_migperf_we),
			.i_wb_addr(wb32_migperf_addr[4-1:0]),
			.i_wb_data(wb32_migperf_data), // 32 bits wide
			.i_wb_sel(wb32_migperf_sel),  // 32/8 bits wide
		.o_wb_stall(wb32_migperf_stall),.o_wb_ack(wb32_migperf_ack), .o_wb_data(wb32_migperf_idata),
		.i_mon_cyc(wbwide_sdram_cyc),
		.i_mon_stb(wbwide_sdram_stb),
		.i_mon_we(wbwide_sdram_we),
		.i_mon_sel(wbwide_sdram_sel),
		.i_mon_stall(wbwide_sdram_stall),
		.i_mon_ack(wbwide_sdram_ack),
		.i_mon_err(wbwide_sdram_err)
		// }}}
	);
...
`endif	// BUSPIC_ACCESS
	...
	wbdown #(
		...
		.i_waddr( wbwide_crossbus_addr[8-1:0]),
