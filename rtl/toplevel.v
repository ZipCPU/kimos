...
`endif
	...
	// Ethernet control (MDIO)
	wire		w_mdio, w_mdwe;
	wire		w_qspi_sck, w_qspi_cs_n, o_qspi_sck;
	wire	[1:0]	qspi_bmod;
	wire	[3:0]	o_qspi_dat, i_qspi_dat;
	wire		s_clk, s_reset;
	...
	main	thedesign(s_clk, s_reset,
		...
		// Quad SPI flash
		w_qspi_cs_n, w_qspi_sck, o_qspi_dat, i_qspi_dat, qspi_bmod,
	...
	xqflex #(
		...
		.i_clk(s_clk), .i_cs_n(w_qspi_cs_n), .i_sck(w_qspi_sck),
		.i_dat(o_qspi_dat), .o_dat(i_qspi_dat), .i_bmod(qspi_bmod),
		//
		.o_cs_n(o_qspi_cs_n), .o_sck(o_qspi_sck), .io_dat(io_qspi_dat)
	);
	...
	PLLE2_BASE #(
		// {{{
		.CLKFBOUT_MULT(8),	// Multiply up to 800MHz
		.CLKFBOUT_PHASE(0.0),
		.CLKIN1_PERIOD(10),
