...
public:
	...
	MAINTB(void) {
		...
		m_eth0->external_mac[3] = 0xef;
		m_eth0->external_mac[4] = 0xda;
		m_eth0->external_mac[5] = 0xd0 | (208 & 0x0f);
		m_eth0->external_ip[0] = 127;
		...
		m_eth0->local_ip[2] = 0x0f & 0x0ff; //  15
		m_eth0->local_ip[3] = 0x1a & 0x0ff; //  26
		m_eth0->local_ipu = 0xc0a80f1a;
		// }}}
		// From mdio
#ifdef	NETCTRL_ACCESS
		...
		// From flash
#ifdef	FLASH_ACCESS
		m_flash = new FLASHSIM(FLASHLGLEN, false, 2, 7);
#endif // FLASH_ACCESS
		// From sdram
#ifdef	SDRAM_ACCESS
	...
	// Evaluating clock eth0_rx_clk

	// sim_eth0_rx_clk_tick() will be called from TESTB<Vmain>::tick()
	//   following any falling edge of clock eth0_rx_clk
	virtual	void	sim_eth0_rx_clk_tick(void) {
		//
		// SIM.TICK tags go here for SIM.CLOCK=eth0_rx_clk
		//
		// SIM.TICK from eth0
		// Simulate the network
		// {{{
		{
			unsigned rxtmp = (*m_eth0)(1,
					m_core->o_eth0_tx_ctl,
					m_core->o_eth0_txd);

			m_core->i_eth0_rx_err = 0;
			if (rxtmp & 0x0100) {
				m_core->i_eth0_rx_dv  = 1;
				m_core->i_eth0_rxd = rxtmp & 0x0ff;
			} else {
				m_core->i_eth0_rx_dv  = 0;
				m_core->i_eth0_rxd = 0x044;
			}
		}
		// }}}
	}
	...
	//
	// Step until clock eth0_rx_clk ticks
	//
	virtual	void	tick_eth0_rx_clk(void) {
		do {
			tick();
		} while(!m_eth0_rx_clk.rising_edge());
	}
