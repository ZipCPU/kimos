...
void sdio_send_voltage_switch(SDIODRV *dev) { // CMD11
	...
	if (0 == (c & SDIO_ERR)) {
		...
		// And another 5ms, for good measure
		for(unsigned k=0; k<150000; k++)
			noop = dev->d_dev->sd_phy;
		...
		for(unsigned k=0; k<30000; k++)
			noop = dev->d_dev->sd_phy;
		// And another 1ms for good measure
		for(unsigned k=0; k<30000; k++)
			noop = dev->d_dev->sd_phy;
...
static	void	sdio_send_tuning_block(SDIODRV *dev) { // CMD19
	...
	for(ph = 0; ph<24; ph++) {
		...
		// Clear the FIFO in case the command fails and doesn't write
		// to the FIFO.  In that case, we want to make sure our FIFO
		// fails to match the pattern.
		sdio_clear_fifo(dev, 0);
		...
		*/
		...
		for(int k=0; (k<16) && vld; k++) {
			...
			if (ph > 10 && 0 == (c & 0x08000)) {
				for(int k=0; k<16; k++) {
					txhex(rxv[k]); txstr(":"); txhex(pat[k]);
					if (rxv[k] != pat[k])
						txstr(" -- FAIL\n");
					else
						txstr("\n");
	...
	if (SDINFO && SDDEBUG) {
		...
		if (vmask == 0) {
			...
			sdio = _sdioscope->s_ctrl;
			sdwb = _sdioscope->s_ctrl;

			TRIGGER_SCOPE;
			txstr(" -- Failed\n");
			txstr("  SDIO-SCOPE : "); txhex(sdio); txstr("\r\n");
			txstr("  SDWB-SCOPE : "); txhex(sdwb); txstr("\r\n");
#else
			txstr(" -- Failed\n");
#endif
		} else {
...
SDIODRV *sdio_init(SDIO *dev) {
	...
	if ((SDDEBUG && SDINFO)) {
		txstr("SDIO INIT: Address = 0x"); txhex((unsigned)dev); txstr("\r\n");
	}
	...
	// if (SDDEBUG && SDINFO) {
	//	txstr("SDIO INIT: Continuing after reset\n");
	// }
	...
	if (SDDEBUG && SDINFO) {
		txstr("SDIO INIT: Sending GO-IDLE, PHY = ");
